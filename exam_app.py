...
import json
import os
import sys  # 新增：用于判断打包环境

# ========== 新增：路径适配工具函数 ==========
def resource_path(relative_path):
    """获取内置资源的绝对路径，兼容开发环境和PyInstaller打包后"""
    try:
        # PyInstaller 打包后运行时的临时解压目录
...
class WelcomePage(tk.Frame):
    def __init__(self, root, start_callback):
        ...
        tk.Label(self, text="思政题库可视化刷题系统", font=("微软雅黑", 24, "bold"), bg=bg_color, fg="#2c3e50").pack(pady=60)
        tk.Label(self, text="支持单选/多选题自动判分 | 实时统计正确率 | 答案永久保存", font=("微软雅黑", 12), bg=bg_color, fg="#34495e").pack(pady=10)
        
        btn_frame = tk.Frame(self, bg=bg_color)
        btn_frame.pack(pady=40)

        tk.Button(btn_frame, text="开始刷题", width=15, height=2, font=("微软雅黑", 14, "bold"), 
        ...
        tk.Label(self, text="内置思政题库，答题记录自动保存在软件同级目录", font=("微软雅黑", 10), bg=bg_color, fg="#7f8c8d").pack(pady=20)
...
class ExamApp:
    def __init__(self, root, question_list, json_path):
        ...
        tk.Button(btn_frame, text="看当前解析", width=10, command=self.check_current_answer, font=("微软雅黑", 10), bg="#f39c12", fg="white").pack(side="left", padx=6)
...
if __name__ == "__main__":
    ...
    def start_exam():
        # 优先加载内置题库
        json_path = resource_path("question_bank.json")
        
        # 如果内置题库不存在（开发环境），弹出选择框
